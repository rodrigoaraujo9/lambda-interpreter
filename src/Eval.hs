...
eval t
 | null (fv t) = eval' t
 | otherwise   = error "free variable occurrence"
...
subst v x (Lambda y e)
  | x == y = Lambda y e
  | elem y (fv v) =
    let z  = fresh (fv e ++ fv v ++ [x,y]) y
        e' = rename y z e
    in Lambda z (subst v x e')
...
subst v x (Let y e1 e2)
  ...
  | elem y (fv v) =
    let z  = fresh (fv e ++ fv v ++ [x,y]) y
        e' = rename y z e
    in Let z (subst v x e1) (subst v x e2')
  | otherwise = Let y (subst v x e1) (subst v x e2)
subst v x (Fix e) = Fix (subst v x e)

-- rename y to z on t (alpha reduction)
rename :: Ident -> Ident -> Term -> Term

-- produce fresh variable for alpha reduction
-- add a number to var string so it easily identifiable
fresh :: Ident -> [Ident]
